...
colors <- sequential_hcl(9, palette = "Reds 2")
colors
...
base_map <- ggplot(rent_burdened) +
  ...
  guides(fill = guide_colorbar(barheight = unit(1, units = "mm"),  
                                 ...
                                 label.position = "bottom",
                                 title.hjust = 0.5)) +
  geom_sf(data = ah_clean, alpha = 0.5, size = 1, color = "orange") +
...
lahd_funding <- ah_clean %>% 
  ...
  annotate(geom = "text", x = 2009, y = 250000000, 
           label = "2015 Directive Order from \nMayor XYZ to facilitate the \nbuilding of affordable housing.",
           color = "white",
           family = "alice") +
  theme_classic() +
  labs(x = 'Year',
  ...
  theme(plot.title = element_text(size = 20, face = "bold", family = "playfairDisplay", hjust = 0.5), # Font size set to 16 and bold.
        ...
        plot.caption = element_text(size = 10, hjust = 0, family = "alice", color = "white"),
        axis.title = element_text(size = 14), # Font size set to 12.
        axis.text = element_text(size = 10, color = "white", family = "alice"),
...
ggplot(compare) +
  geom_boxplot(aes(x = groups, y = percent, fill = status), 
               color = "grey",
               size = 0.1) +
  scale_fill_manual(values = c("#8c2d04", "#fe9929")) +
  coord_flip() +
