...
```{r message=FALSE, warning=FALSE, attr.output=FALSE}
# load API key
...
ah_clean <- ah_raw %>% 
  ...
  st_join(ca_county) %>% 
  select(-c(GEOID, NAME, variable, moe)) # remove these columns
...
la_tract <- ca_county_tracts %>% 
  mutate(census_tract = gsub(", Los Angeles County, California", # elements that you want to remove
                                  "", # replace with blank
                                  NAME)) %>% 
  inner_join(vacancy_LA) %>% 
  rename(median_income = estimate)

la_tract %>% 
  slice_max(order_by = median_income,
            n = 15) %>% 
  ggplot(aes(x = census_tract, 
             y = vacant_units)) +
  geom_col() +
  coord_flip()
